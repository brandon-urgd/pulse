...
requireEnv(['ITEMS_TABLE', 'SESSIONS_TABLE', 'REVISIONS_TABLE', 'CORS_ALLOWED_ORIGINS'])
...
function normalizeItem(raw) {
  ...
  return {
    ...
    hasCompletedRevision: raw.hasCompletedRevision === true,
...
/**
 * Check if an item has at least one completed revision.
 * Queries the REVISIONS_TABLE itemId-index GSI with Limit: 1.
 * Returns false on any failure (fail-open).
 */
async function checkCompletedRevision(itemId) {
  try {
    ...
    log('warn', 'GetItems: failed to check completed revision', { itemId, errorName: err.name })
    return false
  }
}
...
export const handler = async (event) => {
  ...
  try {
    ...
    // For closed/revised items, check for completed revisions
    await Promise.all([
      ...items.filter(i => i.hasPulseCheck && i.pulseCheckGeneratedAt).map(async (item) => {
        try {
      ...
      ...items.filter(i => i.status === 'closed' || i.status === 'revised').map(async (item) => {
        item.hasCompletedRevision = await checkCompletedRevision(item.itemId)
      }),
    ])
